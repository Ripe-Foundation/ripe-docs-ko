...
* GREEN 보유자가 담보를 상환할 수 있고 제3자가 사용자를 [디레버리지](05-deleverage.md)할 수 있습니다
...
기준치는 자산마다 다릅니다. 스테이블코인 등급 자산은 더 높고 변동성 자산은 더 낮습니다. 세 가지 기준이 결합되는 방법은 [세 가지 기준치 이해하기](02-borrowing.md#기준치가-함께-작동하는-방식)를 참조하세요.
...
* **[디레버리지](05-deleverage.md)**: 안정화 풀의 sGREEN 포지션을 상환해 소각하고, 스테이블코인 담보는 오라클 가치로 트레저리에 넘깁니다. 각 $1가 부채 $1를 상쇄합니다.
...
청산 기준에 도달하거나 넘으면 누구나 청산을 실행할 수 있습니다. 거버넌스에는 모든 청산을 일시정지하는 비상 스위치도 있습니다. 이때부터 다음과 같이 진행됩니다.
...
**청산 상태에서 벗어나기.** 포지션이 건전하다고 확인되는 모든 업데이트가 청산 플래그를 해제하고 열린 경매를 취소합니다. 여기서 건전함은 부채가 최대 대출 한도 이하라는 뜻입니다. 상환, 담보 추가, 또는 가격 회복 후 예치나 청구처럼 포지션을 다시 확인하는 작업이 모두 해당합니다. 청산 기준치 위로 조금 올라오는 것만으로는 충분하지 않습니다.
...
재시도에서는 기본 수수료가 0이므로 풀이 스프레드 없이 액면가로 정산합니다. 예치자 입장에서의 설명은 [안정화 풀](../earning-and-rewards/02-stability-pools.md)을 참조하세요.
...
| **청산 수수료** | 13~50% 벌금 | 기본 수수료 + 키퍼 수수료, 담보 잉여액이 상한 |
...
청산된 포지션에 남은 부채가 자동으로 프로토콜 부실 채권이 되는 것은 아닙니다. 거버넌스가 부실 채권을 별도로 기록합니다. 기록된 뒤에는 [본드](../governance-and-economics/03-bonds.md) 구매로 이를 해소할 수 있습니다. 이때 지급되는 RIPE는 프로토콜 전체의 10억 공급 한도를 넘어 발행할 수 있는 유일한 경우입니다. 부족분을 GREEN의 미담보 상태로 남기지 않고 RIPE 보유자 전체가 희석을 통해 분담합니다.
