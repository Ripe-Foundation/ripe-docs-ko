...
3. **이자 지급**: 포지션의 가중 금리가 적용되며, GREEN을 방어해야 할 때는 [변동 금리 가산분](02-borrowing.md#변동-금리)이 더해집니다.
...
* 누군가 [PSM](#6-페그-안정화-모듈psm)의 준비금 스테이블코인을 담보로 GREEN을 발행합니다.
...
전체 메커니즘: [변동 금리](02-borrowing.md#변동-금리).

### 3. 직접 담보 상환 메커니즘

포지션이 [담보 상환 구간](04-liquidations.md#담보-상환-버퍼)에 들어가면 GREEN 보유자는 누구나 해당 포지션을 상대로 담보 상환을 실행할 수 있습니다. GREEN을 $1로 지불하고 오라클 가격의 담보를 받습니다. 할인도 벌금도 없으며, 차용자의 부채는 같은 금액만큼 줄어듭니다. 수량은 내림 처리됩니다.
...
* **지원되는 모든 거래소**: 등록된 통합을 통해 스왑, 유동성 추가 및 제거, 수익 상품 예치를 실행합니다.
...
* **GREEN 발행**: 준비금 스테이블코인을 예치하고 GREEN을 1:1로 받습니다. 설정된 경우 수수료가 차감됩니다. sGREEN을 요청하면 1 GREEN을 초과한 수량을 자동으로 래핑합니다.
...
견적에는 준비금 스테이블코인의 오라클 가격과 1:1 중 사용자에게 더 적은 금액을 주는 쪽이 사용됩니다. 구간별 한도가 발행 및 상환 수량을 제한하고, 출시 단계에는 거버넌스가 허용 목록을 요구할 수 있습니다. 인정된 Underscore 볼트에는 한도, 허용 목록, 수수료가 적용되지 않으며 상환 시 더 유리한 견적을 받습니다. 거버넌스가 수익 볼트를 정했다면 PSM은 유휴 준비금 전액을 그곳에 보관하고 상환에 필요할 때 회수합니다. 상환 한도는 유휴 준비금과 볼트 내 준비금을 합한 총준비금입니다.
...
* 포지션이 청산 기준을 넘으면 누구나 청산을 실행할 수 있으며, 봇이 24시간 감시합니다.
...
* **환율 상승**: 수익이 들어올수록 sGREEN 1개를 더 많은 GREEN으로 상환할 수 있습니다. 리베이스 방식이 아닙니다.
* **프로토콜 수익**: 개시 수수료와 차용자 이자입니다.
* **즉시 유동성**: 언제든 GREEN으로 상환할 수 있습니다.
...
* **시간 가중 방식**: 포지션 수량 × 시간에 따라 포인트가 누적됩니다.
